...
export function buildClaudeWorkerPrompt(input: {
  ...
  return [
    ...
    documentCount > 0 || webpageCount > 0
      ...
      : '',
    documentCount > 0 || webpageCount > 0
      ? '如果预处理文件里明确写了“提取状态：正文缺失或源站未公开正文”或类似提示，请只说明抓取受限；不要根据标题、章节名、作者、站点信息、目录或页面框架推断正文内容。'
      : '',
    ...attachmentLines,
    attachments.length > 0 && !input.taskText.trim()
      ? '用户没有提供额外文字，请先概括附件里的关键信息，再给出有帮助的回复。'
      : '',
    '默认直接回答用户问题，不要附加“总结：做了什么、改了哪些文件、如何验证”这类模板。',
    '只有当你实际修改了工作区文件时，才在最后补一行简短说明改动文件和验证方式。',
    '',
    '用户任务：',
